...
public class ModelSettingJson implements ModelSetting {
    ...
    public String getBackground() {
        if(json.get(BACKGROUND) == null)return null;
        return json.get(BACKGROUND).toString();
    }
    ...
    public boolean getLayout(Map<String, Float> layout) {
        ...
        Map<String,Json.Value> map = json.get(LAYOUT).getMap(null);
    ...
    public String[] getSoundPaths() {
        ...
        Map<String,Json.Value> map=json.get(MOTION_GROUPS).getMap(null);
        for(Map.Entry<String,Json.Value> e : map.entrySet()) {
            //System.out.println(e.getKey() + " : " + e.getValue());
            List<Json.Value> motions=e.getValue().getVector(null);

            for (int i = 0; i < motions.size(); i++) {
                Json.Value motion=motions.get(i);
                Json.Value soundPath=motion.get(SOUND);
                if(soundPath!=null) {
