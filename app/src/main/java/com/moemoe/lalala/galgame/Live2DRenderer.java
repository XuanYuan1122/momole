...
public class Live2DRenderer implements GLSurfaceView.Renderer{
    ...
    public Live2DRenderer( Live2DManager live2DMgr,Context context){
                ...
                if(model.isInitialized() && ! model.isUpdating()) {
                    ...
                    if(mBg == null){
                        if (!TextUtils.isEmpty(model.getBackground())){
                            setupBackground(gl,model.getBackground());
                        }else {
                            setupBackground(gl,Live2DDefine.BACK_IMAGE_NAME);
                        }
                    }else {
    ...
    private void setupBackground(GL10 context,String background) {
        try {
            InputStream in = FileManager.open(background);
