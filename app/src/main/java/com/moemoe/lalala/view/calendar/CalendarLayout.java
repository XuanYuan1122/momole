...
public class CalendarLayout extends LinearLayout{
    ...
    private void init(){
        if(mHeader == null && mContent == null){
            minDistance = getResources().getDimensionPixelOffset(R.dimen.min_distance);
            int headerId = getResources().getIdentifier("vp_calendar","id",getContext().getPackageName());
        ...
        switch (ev.getAction()){
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if(Math.abs(deltaY) <= 8){
                    ...
                    intercepted = false;//横向移动不拦截touch事件
                }else {
                    if(mContent != null && mContent.getTop() == mHeader.getBottom()){
                        intercepted = true;//如果日历完全展示则拦截
                    }else {
                        if(mContent != null && mContent.getTop() - mHeader.getTop() == hideBottom - hideTop){
                            //日历完全收缩
                            if(canChildScrollDown(mContent)){
                                intercepted = false;//底部还可以往下滚动
                            }else {
                                //底部已经滑动到顶，不能在滑动
                                if(deltaY > 0){
                                    //从上向下滑动拦截
