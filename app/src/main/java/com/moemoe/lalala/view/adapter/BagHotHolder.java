...
public class BagHotHolder extends ClickableViewHolder {
    ...
    public BagHotHolder(View itemView) {
        super(itemView);
    }
    ...
    public void createItem(final ShowFolderEntity entity, final int position){
        ...
        if(entity.getCoin() == 0){
            setText(R.id.tv_bag_coin,"免费");
        }else {
            setText(R.id.tv_bag_coin,entity.getCoin() + "节操");
        }
        if(entity.getItems() > 0){
            setText(R.id.tv_bag_num,entity.getItems() + "项");
        }else {
            setText(R.id.tv_bag_num,"");
        }
