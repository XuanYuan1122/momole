...
public class KiraRatingBar extends View {
    ...
    private @ColorInt int borderColor;//边框颜色
    private @ColorInt int fillColor;//填充颜色
    private @ColorInt int backgroundColor;//背景颜色
    private @ColorInt int starBackgroundColor;//星背景颜色
    private @ColorInt int pressedBorderColor;//按下边框颜色
    private @ColorInt int pressedFillColor;//按下填充颜色
    private @ColorInt int pressedBackgroundColor;//按下背景颜色
    private @ColorInt int pressedStarBackgroundColor;//按下星背景颜色
    ...
    public KiraRatingBar(Context context) {
        super(context);
        initView();
    }

    public KiraRatingBar(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        parseAttrs(attrs);
        initView();
    }

    public KiraRatingBar(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void parseAttrs(AttributeSet attrs) {
        TypedArray arr = getContext().obtainStyledAttributes(attrs, R.styleable.KiraRatingBar);

        borderColor = arr.getColor(R.styleable.KiraRatingBar_srb_borderColor, getResources().getColor(R.color.main_cyan));
        fillColor = arr.getColor(R.styleable.KiraRatingBar_srb_fillColor, borderColor);
        starBackgroundColor = arr.getColor(R.styleable.KiraRatingBar_srb_starBackgroundColor, Color.TRANSPARENT);
        backgroundColor = arr.getColor(R.styleable.KiraRatingBar_srb_backgroundColor, Color.TRANSPARENT);

        pressedBorderColor = arr.getColor(R.styleable.KiraRatingBar_srb_pressedBorderColor, borderColor);
        pressedFillColor = arr.getColor(R.styleable.KiraRatingBar_srb_pressedFillColor, fillColor);
        pressedStarBackgroundColor = arr.getColor(R.styleable.KiraRatingBar_srb_pressedStarBackgroundColor, starBackgroundColor);
        pressedBackgroundColor = arr.getColor(R.styleable.KiraRatingBar_srb_pressedBackgroundColor, backgroundColor);

        numberOfStars = arr.getInteger(R.styleable.KiraRatingBar_srb_numberOfStars, 5);

        starsSeparation = arr.getDimensionPixelSize(R.styleable.KiraRatingBar_srb_starsSeparation, (int)valueToPixels(4, Dimension.DP));
        maxStarSize = arr.getDimensionPixelSize(R.styleable.KiraRatingBar_srb_maxStarSize, Integer.MAX_VALUE);
        desiredStarSize = arr.getDimensionPixelSize(R.styleable.KiraRatingBar_srb_starSize, Integer.MAX_VALUE);
        stepSize = arr.getFloat(R.styleable.KiraRatingBar_srb_stepSize, 0.1f);
        starBorderWidth = arr.getFloat(R.styleable.KiraRatingBar_srb_starBorderWidth, 5f);
        starCornerRadius = arr.getFloat(R.styleable.KiraRatingBar_srb_starCornerRadius, 6f);

        rating = normalizeRating(arr.getFloat(R.styleable.KiraRatingBar_srb_rating, 0f));
        isIndicator = arr.getBoolean(R.styleable.KiraRatingBar_srb_isIndicator, false);
        drawBorderEnabled = arr.getBoolean(R.styleable.KiraRatingBar_srb_drawBorderEnabled, true);
        gravity = Gravity.fromId(arr.getInt(R.styleable.KiraRatingBar_srb_gravity, Gravity.Left.id));
    ...
    private void validateAttrs() {
        if (numberOfStars <= 0) {
            throw new IllegalArgumentException(String.format("KiraRatingBar initialized with invalid value for numberOfStars. Found %d, but should be greater than 0", numberOfStars));
        }
        if (desiredStarSize != Integer.MAX_VALUE && maxStarSize != Integer.MAX_VALUE && desiredStarSize
                > maxStarSize) {
            Log.w("KiraRatingBar", String.format("Initialized with conflicting values: starSize is greater than maxStarSize (%f > %f). I will ignore maxStarSize", desiredStarSize, maxStarSize));
        }
        if (stepSize <= 0) {
            throw new IllegalArgumentException(String.format("KiraRatingBar initialized with invalid value for stepSize. Found %f, but should be greater than 0", stepSize));
        }
        if (starBorderWidth <= 0) {
            throw new IllegalArgumentException(String.format("KiraRatingBar initialized with invalid value for starBorderWidth. Found %f, but should be greater than 0",
                    starBorderWidth));
        }
        if (starCornerRadius < 0) {
            throw new IllegalArgumentException(String.format("KiraRatingBar initialized with invalid value for starCornerRadius. Found %f, but should be greater or equal than 0",
                    starBorderWidth));
        }
    }
  ...
  /* ----------- GETTERS AND SETTERS ----------- */
    ...
    public void setStarSize(float starSize) {
        ...
        if (starSize != Integer.MAX_VALUE && maxStarSize != Integer.MAX_VALUE && starSize > maxStarSize) {
            Log.w("KiraRatingBar", String.format("Initialized with conflicting values: starSize is greater than maxStarSize (%f > %f). I will ignore maxStarSize", starSize, maxStarSize));
        }
        // force re-calculating the layout dimension
        requestLayout();
        generateInternalCanvas(getWidth(), getHeight());
    ...
    private void animateRating(AnimationBuilder builder) {
        ...
        ratingAnimator.addListener(new Animator.AnimatorListener() {
            ...
            public void onAnimationEnd(Animator animator) {
                if (ratingListener != null) {
                    ratingListener.onRatingChanged(KiraRatingBar.this, rating, false);
                }
            }
    ...
    private float normalizeRating(float rating) {
        if (rating < 0) {
            Log.w("KiraRatingBar", String.format("Assigned rating is less than 0 (%f < 0), I will set it to exactly 0", rating));
            return 0;
        } else if (rating > numberOfStars) {
            Log.w("KiraRatingBar", String.format("Assigned rating is greater than numberOfStars (%f > %d), I will set it to exactly numberOfStars", rating, numberOfStars));
            return numberOfStars;
        } else {
            return rating;
        }
    ...
    public interface OnRatingBarChangeListener {
         ...
         * lifting the touch.
         *
         * @param KiraRatingBar The RatingBar whose rating has changed.
         * @param rating The current rating. This will be in the range
         *            0..numStars.
         * @param fromUser True if the rating change was initiated by a user's
         *            touch gesture or arrow key/horizontal trackbell movement.
         */
        void onRatingChanged(KiraRatingBar KiraRatingBar, float rating, boolean fromUser);

    }

    public class AnimationBuilder {
        private KiraRatingBar ratingBar;
        ...
        private AnimationBuilder(KiraRatingBar ratingBar) {
