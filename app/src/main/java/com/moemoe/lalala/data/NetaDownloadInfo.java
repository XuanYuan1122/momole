package com.moemoe.lalala.data;

import android.text.TextUtils;
import android.util.Log;

import com.j256.ormlite.dao.Dao;
import com.j256.ormlite.field.DatabaseField;
import com.j256.ormlite.stmt.DeleteBuilder;
import com.j256.ormlite.stmt.UpdateBuilder;
import com.j256.ormlite.table.DatabaseTable;
import com.moemoe.lalala.db.GameDbHelper;
import com.moemoe.lalala.utils.filedownloader.DownloadFileInfo;
import com.moemoe.lalala.utils.filedownloader.FileDownloader;
import com.moemoe.lalala.utils.filedownloader.listener.OnDownloadFileChangeListener;

import java.sql.SQLException;

/**
 * Created by yi on 2016/10/12.
 */
@DatabaseTable(tableName = "tb_game")
public class NetaDownloadInfo implements OnDownloadFileChangeListener {

    public static final String COLUMN_NAME_OF_FIELD_COURSE_URL = "game_url";
    @DatabaseField(generatedId = true, columnName = "_id")
    private Integer mId;//the id of the table

    @DatabaseField(columnName = "game_id", unique = true, canBeNull = false)
    private String mGameId;//the id of the course

    @DatabaseField(columnName = COLUMN_NAME_OF_FIELD_COURSE_URL, unique = true, canBeNull = false)
    private String mGameUrl;//the url of the course

//    @DatabaseField(columnName = "course_cover_url")
//    private String mCourseCoverUrl;//the cover_url of the course

    @DatabaseField(columnName = "game_name")
    private String mGameName;//the name of the course

    private DownloadFileInfo mDownloadFileInfo;//NetaDownloadInfo
    private GameDbHelper mCourseDbHelper;//the DbOpenHelper


    private NetaDownloadInfo() {
        init();
    }

    public NetaDownloadInfo(String gameId, String gameUrl, String courseName,
                            GameDbHelper courseDbHelper) {
        mGameId =gameId;
        mGameUrl = gameUrl;
       // mCourseCoverUrl = courseCoverUrl;
        mGameName = courseName;
        mCourseDbHelper = courseDbHelper;

        init();
    }

    /**
     * init resources
     */
    public void init() {
        // register DownloadFileChangeListener
       // FileDownloader.registerDownloadFileChangeListener(this);
        // init NetaDownloadInfo if has been downloaded
        if (!TextUtils.isEmpty(mGameUrl)) {
            mDownloadFileInfo = FileDownloader.getDownloadFile(mGameUrl);
        }
    }

    /**
     * release resources
     */
    public void release() {
        // unregister
      //  FileDownloader.unregisterDownloadFileChangeListener(this);
    }

    // getters
    public Integer getId() {
        return mId;
    }

    public String getCourseId() {
        return mGameId;
    }

    public String getGameUrl() {
        return mGameUrl;
    }

   // public String getCourseCoverUrl() {
  //      return mCourseCoverUrl;
   // }

    public String getCourseName() {
        return mGameName;
    }

    public DownloadFileInfo getDownloadFileInfo() {
        return mDownloadFileInfo;
    }

    public void setDownloadFileInfo(DownloadFileInfo downloadFileInfo){
        mDownloadFileInfo = downloadFileInfo;
    }

    @Override
    public void onDownloadFileCreated(DownloadFileInfo downloadFileInfo) {
        if (downloadFileInfo != null && downloadFileInfo.getUrl() != null && downloadFileInfo.getUrl().equals
                (mGameUrl)) {

            // add this CoursePreviewInfo in database download record
            //
            // the reason why to save this CoursePreviewInfo in course database is
            // that when the user enter the CourseDownloadFragment,the fragment need to show the course title to the
            // user,however the NetaDownloadInfo in FileDownloader can not provide the course title,also the fragment
            // need to know how many course items need to show,so it is depends on the size of the course database,
            // because FileDownloader is just a tool to download, record and manage all the files which were
            // downloaded from the internet
            try {
                if (mCourseDbHelper == null) {
                    return;
                }
                Dao<NetaDownloadInfo, Integer> dao = mCourseDbHelper.getDao(NetaDownloadInfo.class);
                Dao.CreateOrUpdateStatus status = dao.createOrUpdate(this);
                if (status.isCreated() || status.isUpdated()) {
                    this.mDownloadFileInfo = downloadFileInfo;
                }
            } catch (SQLException e) {
                e.printStackTrace();
            }
        }
    }

    @Override
    public void onDownloadFileUpdated(DownloadFileInfo downloadFileInfo, Type type) {

        if (downloadFileInfo != null && downloadFileInfo.getUrl() != null && downloadFileInfo.getUrl().equals
                (mGameUrl)) {
            if (this.mDownloadFileInfo == null) {
                try {
                    if (mCourseDbHelper == null) {
                        return;
                    }
                    UpdateBuilder builder = mCourseDbHelper.getDao(NetaDownloadInfo.class).updateBuilder();
                    builder.where().eq(NetaDownloadInfo.COLUMN_NAME_OF_FIELD_COURSE_URL, downloadFileInfo.getUrl());
                    int result = builder.update();
                    if (result == 1) {
                        this.mDownloadFileInfo = downloadFileInfo;
                    } else {
                        Dao<NetaDownloadInfo, Integer> dao = mCourseDbHelper.getDao(NetaDownloadInfo.class);
                        Dao.CreateOrUpdateStatus status = dao.createOrUpdate(this);
                        if (status.isCreated() || status.isUpdated()) {
                            this.mDownloadFileInfo = downloadFileInfo;
                        }
                    }
                } catch (SQLException e) {
                    e.printStackTrace();
                }
            }
        }
    }

    @Override
    public void onDownloadFileDeleted(DownloadFileInfo downloadFileInfo) {
        if (downloadFileInfo != null) {
            Log.e("wlf", "onDownloadFileDeleted,downloadFileInfo:" + downloadFileInfo.getUrl());
        } else {
            Log.e("wlf", "onDownloadFileDeleted,downloadFileInfo:" + downloadFileInfo);
        }

        if (downloadFileInfo != null && downloadFileInfo.getUrl() != null && downloadFileInfo.getUrl().equals
                (mGameUrl)) {
            // delete this course preview in database download record
            try {
                if (mCourseDbHelper == null) {
                    return;
                }
                DeleteBuilder builder = mCourseDbHelper.getDao(NetaDownloadInfo.class).deleteBuilder();
                builder.where().eq(NetaDownloadInfo.COLUMN_NAME_OF_FIELD_COURSE_URL, mGameUrl);
                int result = builder.delete();
                if (result == 1) {
                    this.mDownloadFileInfo = null;
                    Log.w("wlf", "onDownloadFileDeleted,downloadFileInfo,delete succeed:" + downloadFileInfo.getUrl());
                }
            } catch (SQLException e) {
                e.printStackTrace();
            }

        }
    }
}
