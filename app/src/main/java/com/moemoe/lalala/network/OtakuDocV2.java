...
public enum OtakuDocV2 {
    ...
    public Call<String> requestNewDocContent(String token
            ,String id){
        return service.requestNewDocContent(token, id);
    }

    public Call<String> requestNewDocTag(String token
            ,String id){
        return service.requestNewDocTag(token, id);
    }
    ...
    public Call<String> giveCoinToDoc(String token
            , String id
            , int coins){
        return service.geiveCoinToDoc(token,id,coins);
    }

    public Call<String> requestCommentsByFloor(String token
            , String id
            , int floor
            , int length
            , String target){
        return service.requestCommentsFromFloor(token,id,floor,length,target);
    }

    public Call<String> favoriteDoc(String token
            ,String id){
        return service.favoriteDoc(token,id);
    }

    public Call<String> cancelfavoriteDoc(String token
            ,String id){
        return service.cancelFavoriteDoc(token,id);
    }

    public Call<String> requestFavoriteDocListUgc(String token
            ,String id
            ,int index
            ,int length){
        return service.requestFavoriteDocListUgc(token,id,index,length);
    }

    public Call<String> requestFavoriteDocListPgc(String token
            ,String id
            ,int index
            ,int length){
        return service.requestFavoriteDocListPgc(token,id,index,length);
    }

    public Call<String> requestQiuMingShanDocList(String token,int index,int len){
        return service.requestQiuMingShanDocList(token,index,len);
    }

    public Call<String> createQiuMingShanDoc(String token,DocPut doc){
        return service.createQiuMingShanDoc(token,doc.toJsonString());
    }
}
