...
public class ProcessLock implements Closeable {
    ...
    static {
        IOUtil.deleteFileOrDir(Utils.app().getDir(LOCK_FILE_DIR, Context.MODE_PRIVATE));
    }
    ...
    public static ProcessLock tryLock(final String lockName, final boolean writeMode) {
        synchronized (LOCK_MAP) {
            ...
            try {
                File file = new File(Utils.app().getDir(LOCK_FILE_DIR, Context.MODE_PRIVATE), customHash(lockName));
